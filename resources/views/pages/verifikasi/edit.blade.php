    ...
    <div class="container py-5">
        ...
        <form action="{{ route('verifikasi.update', $ver->verifikasi_id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                <select name="pendaftar_id" class="form-select" required>
                    ...
                    @foreach ($pendaftar as $p)
                        <option value="{{ $p->pendaftar_id }}" @selected(old('pendaftar_id', $ver->pendaftar_id) == $p->pendaftar_id)>
            ...
            <div class="mb-3">
                ...
                <input type="text" name="petugas" class="form-control" value="{{ old('petugas', $ver->petugas) }}"
                    required>
                @error('petugas')
                    <div class="text-danger small mt-1">{{ $message }}</div>
                @enderror
            ...
            <div class="mb-3">
                ...
                <input type="date" name="tanggal" class="form-control" value="{{ old('tanggal', $ver->tanggal) }}"
                    required>
                @error('tanggal')
                    <div class="text-danger small mt-1">{{ $message }}</div>
                @enderror
                ...
                <div class="d-flex flex-wrap gap-2">
                    @foreach ($ver->media as $m)
                        <div class="position-relative" style="width:120px;">
                            ...
                            <form action="{{ route('verifikasi.media.delete', $m->media_id) }}" method="POST"
